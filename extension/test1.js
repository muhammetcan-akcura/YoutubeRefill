...
function isRefillNeeded(currentFollowers, expectedTotal) {
  return currentFollowers < expectedTotal * 0.99;
}
  ...
  for (const order of orders) {
    try {
      ...
      if (!accountStatus.exists) {
        saveToFile('accountnotfound1.txt', order.id);
        console.log(`[${order.id}] ❌ Hesap bulunamadı -> accountnotfound1.txt`);
        continue;
      }
      
      if (accountStatus.isPrivate) {
        saveToFile('private1.txt', order.id);
        console.log(`[${order.id}] 🔒 Hesap gizli -> private1.txt`);
        continue;
      }
      ...
      if (currentFollowers < startCount) {
        saveToFile('bellowstartcount1.txt', order.id);
        console.log(`[${order.id}] 📉 Takipçi sayısı başlangıç değerinin altında -> bellowstartcount1.txt`);
        continue;
      }
      
      // Yeniden doldurma gerekli mi kontrol edelim
      if (isRefillNeeded(currentFollowers, expected)) {
        saveToFile('refill1.txt',`${order.id} - ${expected - currentFollowers}`);
...
(async () => {
  try {
    ...
    console.log('- refill1.txt: Yeniden doldurma gerekli siparişler');
    console.log('- private1.txt: Gizli hesaplar');
    console.log('- bellowstartcount1.txt: Başlangıç değerinin altındaki takipçi sayıları');
    console.log('- accountnotfound1.txt: Bulunamayan hesaplar');
